...
### `ui/Text` is for button labels; RN's `Text` is for prose

`ui/Text` prepends `text-text text-base` — 16 **and** a 24px line height. The
styles this codebase is migrating set a `fontSize` and nothing else, so routing
plain text through it silently grows every line a step and then needs an
explicit size class to undo that.

React Native's own `Text` takes `className` with no base, so **omitting** a size
class is exact parity by construction. Reach for `ui/Text` when you want
`TextClassContext` — which is what `Button` needs and a paragraph does not.

The same trap sits in Tailwind's named sizes, which are pairs: `text-lg` is 18
*and* 28px of line height, `text-xs` is 12 and 16. Where the original was a bare
`fontSize`, write it bare — `text-[18px]`, not `text-lg`. This cost four
migrations to learn (#114, #116, #117, #118), and `shellChrome.test.tsx` and
`overviewPanels.test.tsx` now pin it rather than describing it.
